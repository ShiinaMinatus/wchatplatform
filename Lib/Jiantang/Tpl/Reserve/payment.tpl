                ...
                <div class="form-group" style="">
                    ...
                    <div class="col-sm-4">
                        <select id="promoSelect" name="promoSelect" class="form-control col-sm-4">
                            <option selected="" promoType="1" promoCost="10" value="">请选择优惠信息</option>
                            {foreach from=$promoInfo item=promo key=key}
                                <option promoType="{$promo.commodity_type}" promoCost="{$promo.commodity_cost}" value="{$promo.commodity_id}">{$promo.commodity_name}优惠</option>
                            {/foreach}
                        </select>
                    </div>
                    <div style="clear: both;"></div>
...
<script>
    ...
    var promoType=$("#promoSelect option:selected").attr("promoType");
    if(promoType=="1"){


    promoMoney=parseFloat(money) *(promoNum*0.1);
}else if(promoType=="2"){
promoMoney=money-promoNum;
promoMoney=promoMoney>=0?promoMoney:0;
}else if (promoType=="4"){
promoMoney=0;
}
