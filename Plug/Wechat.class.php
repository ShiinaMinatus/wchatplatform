...
class Wechat {
    ...
    public function getRevLocation() {
        ...
        $array['Latitude'] = $this->_receive['Latitude'];
        $array['Longitude'] = $this->_receive['Longitude'];
        $array['FromUserName'] = $this->_receive['FromUserName'];
     ...
     * 获取接收事件推送
     */
    public function getRevLocaltionEvent() {
        if (isset($this->_receive['Event'])) {
    ...
    public function sendmessage($array) {
         ...
         * 加载 微信事件类
         */
        $ExtendsPHP = LIB.MODULE_DIR_NAME.'/Extends/'.TOKEN;

        //include $ExtendsPHP;

        $type = $this->getRev()->getRevType();

        $userCode = $this->getRev()->getRevFrom(); //获取微信号码 查询数据库 查看是否已经绑定帐号
        ...
        switch ($type) {
            ...
            case Wechat::MSGTYPE_EVENT:
                ...
                switch($events){
                        ...
                        case 'CLICK':
                            ...
                            $weixinEvent->$eventsKey($array);

                        break;

                        default:

                            $weixinEvent->$events($array);
                }
            ...
            case Wechat::MSGTYPE_VOICE:
                ...
                $sendData = $this->getRev();

                $this->text('ccc')->reply();
                //file_put_contents("voice.txt", var_export($weixin_location, TRUE));
                break;


            case Wechat::MSGTYPE_LOCATION:

                $this->text('签到成功!')->reply();

            default:
                $this->text($type)->reply();
        }
        ...
        if (!empty($WeixinArray[$text])) {
            ...
            if (!empty($array)) {
                ...
                if ($array['join'] != '') {
                    ...
                    $join_number = $array['join'];
                    $join_array = explode(',', $join_number);
                    ...
                    if (count($join_array) > 0) {
                        ...
                        foreach ($join_array as $k => $v) {
                            ...
                            if (!empty($WeixinArray[$v])) {

                                $applytext.=($v) . '.' . $WeixinArray[$v]['name'] . PHP_EOL;
                            }
                        }
                    }
                } else {
            ...
            switch ($array['type']) {
                ...
                case 'image':
                    ...
                    $image_json = json_decode($applytext, true);

                    $this->news($image_json)->reply();
